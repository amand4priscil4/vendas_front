// Configuração da API
const API_BASE_URL = 'https://vendas-mercearia.onrender.com';
...
function atualizarSelectProdutos() {
  ...
  select.innerHTML =
    ...
    produtos
      .map(
        produto =>
          `<option value="${produto.id}">${produto.nome} - R$ ${produto.preco_normal.toFixed(
            2
          )}</option>`
      )
      .join('');
      ...
      cliente => `
        <div class="cliente-card">
            ...
            <div class="cliente-info">
                ...
                ${cliente.cpf ? `<div><i class="fas fa-id-card"></i> ${cliente.cpf}</div>` : ''}
                ${
                  cliente.endereco
                    ? `<div><i class="fas fa-map-marker-alt"></i> ${cliente.endereco}</div>`
                    : ''
                }
            </div>
            <div class="card-actions">
...
async function salvarCliente(e) {
  ...
  const dados = {
    ...
    cpf: document.getElementById('cpfCliente').value || null,
    telefone: document.getElementById('telefoneCliente').value || null,
    endereco: document.getElementById('enderecoCliente').value || null
  };
...
function renderizarVendasRecentes() {
  ...
  container.innerHTML = vendasRecentes
    .map(
      venda => `
        <div class="venda-item">
            <div class="venda-info">
                <div>
                    ...
                    <span class="venda-pagamento">${formatarFormaPagamento(
...
async function carregarVendasPendentes() {
  try {
    ...
    elements.vendasPendentes.innerHTML =
      '<div class="loading">Erro ao carregar vendas pendentes</div>';
  }
}
              ...
              .map(
                ...
                <div class="venda-item">
                    <div class="venda-info">
                        ...
                        <div class="venda-data">${formatarData(venda.created_at)} (${Math.floor(
                  venda.dias_pendente
                )} dias)</div>
                    </div>
                    <div class="venda-valor">R$ ${venda.valor_total.toFixed(2)}</div>
